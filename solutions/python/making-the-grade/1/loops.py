...
def round_scores(student_scores):
    ...
    for scores in student_scores:
        test_scores.append(round(scores))
    return test_scores
...
def letter_grades(highest):
    ...
    for i in range(4):
        graded_list.append(41+interval*i)
    return graded_list
...
def student_ranking(student_scores, student_names):
    ...
    for index , name in enumerate(student_names):
        ranks.append(f'{index+1}. {name}: {student_scores[index]}')
    return ranks   
...
def perfect_score(student_info):
    ...
    for index, (name , score) in enumerate(student_info):
        if score==100:
